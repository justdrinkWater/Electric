...
function insertRowInTable(sTable, iNum) {
	...
	oTd0.innerHTML = "<input onclick=\"getSelectRow(event.srcElement,document.all.selectrow)\" type=\"checkbox\" value=\""
			+ iNum
			+ "\" name=\"selprocess"
			+ index
			+ "\" id=\"selprocess\"  >  "
			+ "<input  type=\"hidden\" name=\"stepForm["
			+ index
			+ "].stepnum\" value=\""
			+ iNum
			+ "\">"
	...
	oTd2.innerHTML = "<TEXTAREA name=\"stepForm["
			+ index
			+ "].teststep\" maxlength=\"1024\" style=\"behavior:url(../htc/maxlength.htc)\"  onchange=\"limitarea(this,1024)\" ></TEXTAREA>";

	//
	var oTd3 = oTr.insertCell(3);
	oTd3.setAttribute("width", "23%");
	oTd3.setAttribute("align", "center");
	oTd3.innerHTML = "<TEXTAREA name=\"stepForm["
			+ index
			+ "].condition\" maxlength=\"332\" style=\"behavior:url(../htc/maxlength.htc)\"  onchange=\"limitarea(this,332)\" ></TEXTAREA>";

	var oTd4 = oTr.insertCell(4);
	oTd4.setAttribute("width", "26%");
	oTd4.setAttribute("align", "center");
	oTd4.innerHTML = "<TEXTAREA name=\"stepForm["
			+ index
			+ "].resuexpect\" maxlength=\"332\" style=\"behavior:url(../htc/maxlength.htc)\"  onchange=\"limitarea(this,332)\" ></TEXTAREA>";

	var oTd5 = oTr.insertCell(5);
	oTd5.setAttribute("width", "14%");
	oTd5.setAttribute("align", "center");
	// oTd5.innerHTML="<img border=\"0\" src=\"../images/delete.gif\"
	// width=\"18\" height=\"18\"
	// onclick=\"deleteCaseStepClient('processelisttable',event.srcElement)\">";
...
function openWindowWithName(sHref, strWidth, strHeight, sName) {
	...
	strRef = strRef + "left=" + strLeft + "px,top=" + strTop + "px,";
	strRef = strRef
			+ "resizable=no,scrollbars=yes,status=yes,toolbar=no,systemmenu=no,location=no,borderSize=thin";// channelmode,fullscreen
