...
function checkDatetime(checktext) {
	...
	if (checktext != "") {
		...
		gone = datetime.substring(datetime.indexOf("-"),
				datetime.indexOf("-") + 1);
		month = datetime.substring(datetime.indexOf("-") + 1, datetime
				.lastIndexOf("-"));
		if (isNaN(month) == true) {
			return false;
		}
		gtwo = datetime.substring(datetime.lastIndexOf("-"), datetime
				.lastIndexOf("-") + 1);
		day = datetime
				.substring(datetime.lastIndexOf("-") + 1, datetime.length);
		if (isNaN(day) == true) {
			return false;
		}
		if ((gone == "-") && (gtwo == "-")) {
...
function checkBirthday(checktext) {
	...
	if (checktext != "") {
		...
		month = datetime.substring(datetime.indexOf("-") + 1, datetime
				.lastIndexOf("-"));
		if (isNaN(month) == true || dateObj.getMonth() < month
		...
		day = datetime
				.substring(datetime.lastIndexOf("-") + 1, datetime.length);
		if (isNaN(day) == true || dateObj.getDay() < day
				&& dateObj.getMonth() < month && dateObj.getYear() < year) {
			return false;
		}
		if ((gone == "-") && (gtwo == "-")) {
			if (month < 1 || month > 12) {
...
function checkIdentityNumber(strNumber) {
	...
	if ((!isNaN(strNumber) || (!isNaN(strNumber.substr(0, 17)) && strNumber
			.substr(17, 18) == "x"))
			&& (strNumber.length == 15 || strNumber.length == 18))
		return true;
	return false;
}
