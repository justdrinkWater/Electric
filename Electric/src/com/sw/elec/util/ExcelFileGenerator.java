	...
	public HSSFWorkbook createWorkbook() {
		...
		for (int i = 1; i <= sheetNum; i++) {
			...
			for (int j = 0; j < fieldName.size(); j++) {
				HSSFCell cell = headRow.createCell((int) j);
				// 添加样式
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
				// 添加样式
				// 设置所有单元格的宽度
				sheet.setColumnWidth((int) j, (int) 6000);
			...
			for (int k = 0; k < (rows < SPLIT_COUNT ? rows : SPLIT_COUNT); k++) {
				HSSFRow row = sheet.createRow((int) (k + 1));
				// 将数据内容放入excel单元格
				ArrayList<String> rowList = (ArrayList<String>) fieldData.get((i - 1)
						* SPLIT_COUNT + k);
				for (int n = 0; n < rowList.size(); n++) {
					HSSFCell cell = row.createCell((int) n);
